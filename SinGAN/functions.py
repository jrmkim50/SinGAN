...
def generate_dir2save(opt):
    ...
    if (opt.mode == 'train') | (opt.mode == 'SR_train'):
        dir2save = 'TrainedModels/%s/scale_factor=%f,alpha=%d,min_size=%d,num_layers=%d' % (opt.input_name[:-4], opt.scale_factor_init,opt.alpha,opt.min_size,opt.num_layer)
...
def post_config(opt):
    ...
    opt.device = torch.device("cpu" if opt.not_cuda else f"cuda:{opt.device_num}")

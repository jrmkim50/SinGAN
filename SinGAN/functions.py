...
def generate_dir2save(opt):
    ...
    if (opt.mode == 'train') | (opt.mode == 'SR_train'):
        dir2save = f'{f"{opt.out}/" if opt.out != "Output" else ""}TrainedModels/{opt.input_name[:-4]}/scale_factor={opt.scale_factor_init:.3f},num_layers={opt.num_layer},sim_alpha={opt.sim_alpha:.3f},sim_boundary={opt.sim_boundary},sim_boundary_type={opt.sim_boundary_type},use_attn_g={opt.use_attention_g},use_attn_end_g={opt.use_attention_end_g},use_attn_d={opt.use_attention_d},use_attn_end_d={opt.use_attention_end_d},nfc={opt.nfc_init},min_size={opt.min_size},few_gan={opt.few_gan},num_layer_d={opt.num_layer_d_init}{"_groupnorm" if opt.groupnorm else ""}{",prelu" if opt.prelu else ""}{",train_last_longer" if opt.train_last_layer_longer else ""}{f",train_first_longer={opt.train_first_layers_longer}" if opt.train_first_layers_longer else ""}{",split" if opt.split_image else ""}{",harmonic" if opt.harmonic_ssim else ""}{",warm_g" if opt.warmup_g else ""}{",warm_d" if opt.warmup_d else ""}{",spectral_g" if opt.spectral_norm_g else ""}{",spectral_d" if opt.spectral_norm_d else ""}{",one_ssim" if opt.sim_loss_one_image else ""}{f",vgg_axis={opt.vgg_axis}" if opt.vgg_axis else ""}{f",update_in_one_go" if opt.update_in_one_go else ""}{opt.config_tag}'
...
def post_config(opt):
    ...
    opt.num_layer_d_init = opt.num_layer_d

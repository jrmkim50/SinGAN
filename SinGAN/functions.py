...
def generate_dir2save(opt):
    ...
    if (opt.mode == 'train') | (opt.mode == 'SR_train'):
        if opt.split_images:
            # dir2save = 'TrainedModels/%s_split/scale_factor=%.3f,num_layers=%d,sim_alpha=%.3f,sim_boundary=%d,sim_boundary_type=%s,sim_type=%s,alpha=%.3f,use_attn=%d,use_attn_end=%d,nfc=%d,penalize_mask=%d,pd=%d,min_size=%d' % (opt.input_name[:-4], opt.scale_factor_init,opt.num_layer,opt.sim_alpha,opt.sim_boundary,opt.sim_boundary_type,opt.sim_type,opt.alpha,opt.use_attention,opt.use_attention_end,opt.nfc_init,opt.penalize_mask,opt.penalize_diversity,opt.min_size)
            dir2save = 'TrainedModels/%s_split/scale_factor=%.3f,num_layers=%d,sim_alpha=%.3f,sim_boundary=%d,sim_boundary_type=%s,sim_type=%s,alpha=%.3f,use_attn=%d,use_attn_end=%d,nfc=%d,penalize_mask=%d,min_size=%d' % (opt.input_name[:-4], opt.scale_factor_init,opt.num_layer,opt.sim_alpha,opt.sim_boundary,opt.sim_boundary_type,opt.sim_type,opt.alpha,opt.use_attention,opt.use_attention_end,opt.nfc_init,opt.penalize_mask,opt.min_size)
        else:
            # dir2save = 'TrainedModels/%s/scale_factor=%.3f,num_layers=%d,sim_alpha=%.3f,sim_boundary=%d,sim_boundary_type=%s,sim_type=%s,alpha=%.3f,use_attn=%d,use_attn_end=%d,nfc=%d,penalize_mask=%d,pd=%d,min_size=%d' % (opt.input_name[:-4], opt.scale_factor_init,opt.num_layer,opt.sim_alpha,opt.sim_boundary,opt.sim_boundary_type,opt.sim_type,opt.alpha,opt.use_attention,opt.use_attention_end,opt.nfc_init,opt.penalize_mask,opt.penalize_diversity,opt.min_size)
            dir2save = 'TrainedModels/%s/scale_factor=%.3f,num_layers=%d,sim_alpha=%.3f,sim_boundary=%d,sim_boundary_type=%s,sim_type=%s,alpha=%.3f,use_attn=%d,use_attn_end=%d,nfc=%d,penalize_mask=%d,min_size=%d' % (opt.input_name[:-4], opt.scale_factor_init,opt.num_layer,opt.sim_alpha,opt.sim_boundary,opt.sim_boundary_type,opt.sim_type,opt.alpha,opt.use_attention,opt.use_attention_end,opt.nfc_init,opt.penalize_mask,opt.min_size)
        ...
        if opt.split_images:
            # dir2save = '%s/RandomSamples/%s_split/scale_factor=%.3f,num_layers=%d,sim_alpha=%.3f,sim_boundary=%d,sim_boundary_type=%s,sim_type=%s,alpha=%.3f,use_attn=%d,use_attn_end=%d,nfc=%d,penalize_mask=%d,pd=%d,min_size=%d/gen_start_scale=%d' % (opt.out,opt.input_name[:-4], opt.scale_factor_init,opt.num_layer,opt.sim_alpha,opt.sim_boundary,opt.sim_boundary_type,opt.sim_type,opt.alpha,opt.use_attention,opt.use_attention_end,opt.nfc_init,opt.penalize_mask,opt.penalize_diversity,opt.min_size, opt.gen_start_scale)
            dir2save = '%s/RandomSamples/%s_split/scale_factor=%.3f,num_layers=%d,sim_alpha=%.3f,sim_boundary=%d,sim_boundary_type=%s,sim_type=%s,alpha=%.3f,use_attn=%d,use_attn_end=%d,nfc=%d,penalize_mask=%d,min_size=%d/gen_start_scale=%d' % (opt.out,opt.input_name[:-4], opt.scale_factor_init,opt.num_layer,opt.sim_alpha,opt.sim_boundary,opt.sim_boundary_type,opt.sim_type,opt.alpha,opt.use_attention,opt.use_attention_end,opt.nfc_init,opt.penalize_mask,opt.min_size, opt.gen_start_scale)
        else:
            # dir2save = '%s/RandomSamples/%s/scale_factor=%.3f,num_layers=%d,sim_alpha=%.3f,sim_boundary=%d,sim_boundary_type=%s,sim_type=%s,alpha=%.3f,use_attn=%d,use_attn_end=%d,nfc=%d,penalize_mask=%d,pd=%d,min_size=%d/gen_start_scale=%d' % (opt.out,opt.input_name[:-4], opt.scale_factor_init,opt.num_layer,opt.sim_alpha,opt.sim_boundary,opt.sim_boundary_type,opt.sim_type,opt.alpha,opt.use_attention,opt.use_attention_end,opt.nfc_init,opt.penalize_mask,opt.penalize_diversity,opt.min_size, opt.gen_start_scale)
            dir2save = '%s/RandomSamples/%s/scale_factor=%.3f,num_layers=%d,sim_alpha=%.3f,sim_boundary=%d,sim_boundary_type=%s,sim_type=%s,alpha=%.3f,use_attn=%d,use_attn_end=%d,nfc=%d,penalize_mask=%d,min_size=%d/gen_start_scale=%d' % (opt.out,opt.input_name[:-4], opt.scale_factor_init,opt.num_layer,opt.sim_alpha,opt.sim_boundary,opt.sim_boundary_type,opt.sim_type,opt.alpha,opt.use_attention,opt.use_attention_end,opt.nfc_init,opt.penalize_mask,opt.min_size, opt.gen_start_scale)

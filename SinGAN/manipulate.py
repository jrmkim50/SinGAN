    ...
    for G,Z_opt,noise_amp in zip(Gs,Zs,NoiseAmp):
        ...
        for i in range(0,num_samples,1):
            ...
            if n == len(reals)-1:
                if opt.mode == 'train':
                    if opt.split_images:
                        dir2save = '%s/RandomSamples/%s_split/scale_factor=%.3f,num_layers=%d,sim_alpha=%.3f,sim_boundary=%d,sim_boundary_type=%s,sim_type=%s,alpha=%.3f,use_attn=%d,use_attn_end=%d,nfc=%d/gen_start_scale=%d' % (opt.out,opt.input_name[:-4], opt.scale_factor_init,opt.num_layer,opt.sim_alpha,opt.sim_boundary,opt.sim_boundary_type,opt.sim_type,opt.alpha,opt.use_attention,opt.use_attention_end,opt.nfc_init, gen_start_scale)
                    else:
                        dir2save = '%s/RandomSamples/%s/scale_factor=%.3f,num_layers=%d,sim_alpha=%.3f,sim_boundary=%d,sim_boundary_type=%s,sim_type=%s,alpha=%.3f,use_attn=%d,use_attn_end=%d,nfc=%d/gen_start_scale=%d' % (opt.out,opt.input_name[:-4], opt.scale_factor_init,opt.num_layer,opt.sim_alpha,opt.sim_boundary,opt.sim_boundary_type,opt.sim_type,opt.alpha,opt.use_attention,opt.use_attention_end,opt.nfc_init, gen_start_scale)
                else:
                    dir2save = functions.generate_dir2save(opt)
                try:
